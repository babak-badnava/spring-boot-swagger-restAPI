        ...
        for (Class<?> clazz = handlerType; clazz != Object.class; clazz = clazz.getSuperclass()) {
            if (clazz.isAnnotationPresent(RequestMapping.class)) {
                if (springPath) {
                    ...
                    // TODO handle other elements in the array if necessary
                } else {
                    springPath = true;
                }
            }
        }
        superclassUrlPatterns = Lists.reverse(superclassUrlPatterns);
        if (!superclassUrlPatterns.isEmpty()) {
            RequestMappingInfo superclassRequestMappingInfo =
                    new RequestMappingInfo(
                            methodMapping.getName(),
                            new PatternsRequestCondition(String.join("", superclassUrlPatterns)),
                            methodMapping.getMethodsCondition(),
